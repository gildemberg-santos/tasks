...
class Tasks extends Component {
    ...
    async deleteTaskAll() {
        if (window.confirm(`Are you sure you want to delete?`)) {
            await this.state.tasks.filter((task) => task.done == true).forEach(element => {
    ...
    async loadTasks() {
        let respose = await fetch('http://localhost:3001/tasks');
        ...
        return (
            <Row>
                ...
                <Col xs={{ span: 8, offset: 2 }} className="tasks_list">
                    ...
                    <List loadTasks={this.loadTasks} tasks={this.state.tasks.filter((task) => task.done == true)} />
                    <Button variant="red" className="float-right remove_tasks_btn" onClick={() => this.deleteTaskAll()}>Remove all tasks</Button>
                </Col>
            </Row>
        );
    }
